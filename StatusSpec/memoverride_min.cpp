...
#if _MSC_VER >= 1900
	void *_calloc_base(size_t count, size_t nSize)
	{
		void *pMem = AllocUnattributed(count * nSize);
		memset(pMem, 0, count * nSize);
		return pMem;
	}
	void *_recalloc_base(void *pMem, size_t count, size_t nSize)
	{
...
#define AFRET(var)  &var
	...
	{
		static int dummy = _CRTDBG_ALLOC_MEM_DF;
		return &dummy;
	}
...
#if !defined(SUPPRESS_INVALID_PARAMETER_NO_INFO) && _MSC_VER < 1900
...
constexpr auto MAX_LC_LEN = (MAX_LANG_LEN + MAX_CTRY_LEN + MAX_MODIFIER_LEN + 3);
...
struct _tiddata
{
	...
	int         _tfpecode;      /* float point exception code */
								...
								* the thread */
	__crt_multibyte_data*  ptmbcinfo;
